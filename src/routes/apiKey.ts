...
app.post('/', sValidator('json', createKeySchema), async c => {
  ...
  // As basis of jwt token we get the user id and then genrated random api key with hash and prefix, that api-key will store in api key table for that particualr user, so that user may contains many api keys
